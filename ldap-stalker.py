...
def check_control_user(old_entries, new_entries):
    ...
    for entry_id in new_entries:
        ...
        if 'entryUUID' in entry and CONTROL_UUID in entry['entryUUID']:
            old_entry = old_entries[entry_id]
            if not CONTROL_USER_ATTRIBUTE or len(CONTROL_USER_ATTRIBUTE) == 0:
                for attribute in entry.keys():
                    if entry[attribute] != old_entry[attribute]:
                        control_user_found = True
                        break
            else:
                if entry[CONTROL_USER_ATTRIBUTE] != old_entry[CONTROL_USER_ATTRIBUTE]:
                    control_user_found = True
...
def compare_ldap_entries(old_entries, new_entries):
    if len(CONTROL_UUID) > 0 and not check_control_user(old_entries, new_entries):
        send_to_slack("ERROR: Cannot find the control user's change.")
        sys.exit(1)

    # XXX: The next five lines do not consider ignored UUIDs or ignored attributes.
