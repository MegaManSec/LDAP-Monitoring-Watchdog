...
from ldap_watchdog import config

if config.SLACK_WEBHOOK and len(config.SLACK_WEBHOOK) > 0:
    import requests
...
def col(op_type):
    ...
    - str: ANSI color code.
    """
    if config.DISABLE_COLOR_OUTPUT:
        return ''
...
def retrieve_ldap():
    ...
    """
    entries = {}
    server = Server(config.LDAP_SERVER, use_ssl=config.LDAP_USE_SSL, get_info=ALL)
    if config.LDAP_USERNAME and config.LDAP_PASSWORD:
        conn = Connection(server, user=config.LDAP_USERNAME, password=config.LDAP_PASSWORD)
        if not conn.bind():
    ...
    conn.search(search_base=config.BASE_DN, search_filter=config.SEARCH_FILTER, search_scope=SUBTREE, attributes=config.SEARCH_ATTRIBUTE)
...
def generate_message(dn_uuid, op_type, changes):
    ...
    bl = f"{config.SLACK_BULLETPOINT}{op_type}"
    ...
    if op_type == 'modify':
        for additions in changes['additions']:
            for key, vals in additions.items():
                for val in vals:
                    slack_msg += f"{config.SLACK_BULLETPOINT}add *'{key}'* to *'{val}'*\n"
                    print_msg += f"{config.SLACK_BULLETPOINT}add '{col('modify')}{key}{rst_col}' to '{col('add')}{val}{rst_col}'\n"
        for removals in changes['removals']:
            for key, vals in removals.items():
                for val in vals:
                    slack_msg += f"{config.SLACK_BULLETPOINT}delete *'{key}'* was _'{val}'_\n"
                    print_msg += f"{config.SLACK_BULLETPOINT}delete '{col('modify')}{key}{rst_col}' was '{col('delete')}{val}{rst_col}'\n"
        for modifications in changes['modifications']:
            for key, val in modifications.items():
                slack_msg += f"{config.SLACK_BULLETPOINT}modify *'{key}'* to *'{val[1]}'* was _'{val[0]}'_\n"
                print_msg += f"{config.SLACK_BULLETPOINT}modify '{col('modify')}{key}{rst_col}' to '{col('add')}{val[1]}{rst_col}' was '{col('delete')}{val[0]}{rst_col}'\n"
    elif op_type == 'delete':
...
def send_to_slack(message):
    ...
    - None
    """
    if config.SLACK_WEBHOOK is None or len(config.SLACK_WEBHOOK) == 0:
        return
    ...
    data = {'text': message}
    requests.post(config.SLACK_WEBHOOK, headers=headers, data=json.dumps(data))
...
def check_control_user(old_entries, new_entries):
    ...
    if config.CONTROL_UUID not in new_entries or config.CONTROL_UUID not in old_entries:
      return control_user_found

    new_entry = new_entries[config.CONTROL_UUID]
    old_entry = old_entries[config.CONTROL_UUID]

    if config.CONTROL_USER_ATTRIBUTE and len(config.CONTROL_USER_ATTRIBUTE) > 0:
        # If using CONTROL_USER_ATTRIBUTE, only compare that attribute as long as it exists in both new_entry and old_entry.
        if config.CONTROL_USER_ATTRIBUTE in new_entry and config.CONTROL_USER_ATTRIBUTE in old_entry and new_entry[config.CONTROL_USER_ATTRIBUTE] != old_entry[config.CONTROL_USER_ATTRIBUTE]:
...
def compare_ldap_entries(old_entries, new_entries):
    ...
    - None
    """
    if len(config.CONTROL_UUID) > 0 and not check_control_user(old_entries, new_entries):
    ...
    for uuid in old_entries.keys() & new_entries.keys():
        if uuid in config.IGNORED_UUIDS:
        ...
        if old_entries[uuid] != new_entries[uuid]:
            ...
            for change_type in ["additions", "modifications", "removals"]:
                for ignored_attr_name in config.IGNORED_ATTRIBUTES:
            ...
            for change_type in ["additions", "modifications", "removals"]:
                for ignored_attr_name, ignored_attr_list in config.CONDITIONAL_IGNORED_ATTRIBUTES.items():
